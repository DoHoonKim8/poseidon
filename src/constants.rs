...
const MDS_MATRIX_CIRC: [[u128; 12]; 12] =
    [
...
const MDS_MATRIX_DIAG: [[u128; 12]; 12] =
    [
...
const ROUND_CONSTANTS: [[u128; 12]; 30] =
    [
        [
...
pub fn mds_matrix<F: FieldExt, const T: usize, const RATE: usize>() -> MDSMatrix<F, T, RATE> {
    let mds =
        MDS_MATRIX_CIRC
            .iter()
            .zip(MDS_MATRIX_DIAG.iter())
            .map(|(row_circ, row_diag)| {
                row_circ
                    .iter()
                    .zip(row_diag.iter())
                    .map(|(e_circ, e_diag)| F::from_u128(e_circ + e_diag))
                    .collect()
            })
            .collect();
...
pub fn get_round_constants<F: FieldExt, const T: usize>() -> Vec<[F; T]> {
    ROUND_CONSTANTS
        .to_vec()
        .iter()
        .map(|l| {
